...
一般来说，Redux 建议带有副作用的代码应该是 action 创建过程的一部分。虽然该逻辑 _可以_ 在 UI 组件内执行，但通常将逻辑提取到可重用函数中以便可以从多个位置调用相同的逻辑（一个 action creator 函数）。
