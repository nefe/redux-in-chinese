...
const features = [
  ...
  },
  {
    content: (
      <p>
        集中管理应用的状态和逻辑可以让你开发出强大的功能，如 <strong>撤销/重做</strong>、{' '}
        <strong>状态持久化</strong> 等等。
      </p>
    ),
    image: (
      <svg
  ...
  },
  {
    content: (
      <p>
        Redux DevTools 让你轻松追踪到{' '}
        <strong>
          应用的状态在何时、何处以及如何改变
        </strong>
        。Redux 的架构让你记下每一次改变，借助于{' '}
        <strong>"时间旅行调试"</strong>，你甚至可以把完整的错误报告发送给服务器。
      </p>
    ),
    image: (
      <svg
  ...
  },
  {
    content: (
      <p>
        Redux <strong>可与任何 UI 层框架搭配使用</strong>，并且有{' '}
        <strong>庞大的插件生态</strong> 来实现你的需求.
      </p>
    ),
    image: (
      <svg
...
function Home() {
  ...
  return (
    <Layout
      title={`${siteConfig.title} - JavaScript 应用的状态容器，提供可预测的状态管理。`}
      description="JavaScript 应用的状态容器，提供可预测的状态管理。"
    >
